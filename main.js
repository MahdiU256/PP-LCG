...
function checkAns(e) {
	...
	// Game Over
	if (
		boxArr[0][0].value == comboArr[0] &&
		boxArr[1][0].value == comboArr[1] &&
		boxArr[2][0].value == comboArr[2]
	) {
		gameOver();
	}
}
...
function gameOver() {
	// Reveal the combination
	$('#combination').css('display', 'block');

	// Stop displaying the game
	$('#game').css('display', 'none');

	// Show some 'Game Over' text
	$('<span id="gameOver"></span>').insertBefore($('#combination'));
	$('#gameOver').html('Well Done!');
	$('#gameOver').css({
